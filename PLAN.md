...
Acceptance: run the same stored workflow twice with different inputs. The second agent receives the first agent's real output, and each execution has an independent persisted trace.
...
3. Search ANS for each capability using fields actually supported by its API. Normalize results into the internal agent contract.
...
- Ordered steps showing agent name, purpose, protocol, and ANS provenance.
...
4. Re-resolve agents before execution and record the endpoint and identity evidence used. If identity or compatibility materially changes, stop and require review.
...
| Object | Purpose |
| --- | --- |
| DiscoveredAgent | ANS identity, protocol, capabilities, endpoint, discovery timestamp, raw evidence reference |
...
| `POST /api/discover` | Capabilities → compatible ANS candidates and proposal |
...
- Which LLM provider and hosting environment are available to the team?
