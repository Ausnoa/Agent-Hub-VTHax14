...
export async function runDefinition(input: Definition, source: string, generate = structuredPlan): Promise<string> {
  ...
  if(agent.template==='summary') {
    const result=await generate(payload,`${rules} Summarize sourceText into a brief, key points, and only explicitly stated action items. Preserve uncertainty. Never invent owners or deadlines. Use an empty actionItems array if no tasks are assigned.`,summarySchema,{maxOutputTokens:4000});
    return `Brief\n${result.brief}\n\nKey points\n${result.keyPoints.map(p=>`- ${p}`).join('\n')||'None stated.'}\n\nAction items\n${result.actionItems.map(p=>`- ${p}`).join('\n')||'None explicitly stated.'}`;
  }
  if(agent.template==='extract') {
    const result=await generate(payload,`${rules} Extract exactly the requested fields from sourceText. Return each requested field name exactly once. Values must be literal source excerpts, or null if missing.`,extractionSchema,{maxOutputTokens:4000});
  ...
  const result=await generate(payload,`${rules} Answer sourceText as a question using reference only. For supported answers include exact reference excerpts as evidence. If reference cannot answer, set supported=false and evidence=[]; do not guess.`,answerSchema,{maxOutputTokens:4000});
