...
export function repository({ client, userId }: Identity) {
  return {
    ...
    async history(id: string) {
      const { data, error } = await client.from('agent_test_runs').select('id,agent_id,status,output,created_at').eq('owner_id', userId).eq('agent_id', id).order('created_at', { ascending: false }).limit(20);
