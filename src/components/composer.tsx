  ...
  return <div className="shell">
    <aside className="sidebar">
      ...
      <nav aria-label="Main navigation">
        ...
        <button className={tab === "discover" ? "nav-item active" : "nav-item"} onClick={() => { setTab("discover"); action("Loading catalog", async () => setCatalog(await api<CatalogEntry[]>("catalog"))); }}><span>◎</span> Pilot catalog</button>
      </nav>
      ...
      <main>
        ...
        {tab === "builder" && !proposal && !agent && <>
          ...
          <section className="builder-card"><div className="card-heading"><span className="number-label">01</span><h2>What should your agent do?</h2><span className="badge">NEW COMPOSITION</span></div>
            ...
            <div className="mode-row"><div className="mode-toggle" aria-label="Composition mode"><button className={mode === "pilot" ? "selected" : ""} onClick={() => setMode("pilot")}>Pilot catalog</button><button className={mode === "demo" ? "selected" : ""} onClick={() => setMode("demo")}>Offline demo</button><button className={mode === "live" ? "selected" : ""} onClick={() => setMode("live")}>ANS catalog</button></div><button className="primary" disabled={!!busy || description.trim().length < 10} onClick={() => action("Planning capabilities", async () => setProposal(await api<Proposal>("proposals", { description, mode })))}>{busy || "Build my agent"}<span>↗</span></button></div>
            <p className="mode-note">{mode === "pilot" ? `Real LLM planning → small indexed catalog → local A2A test agents. Execution uses supplied notes, not live research. Planner ${plannerConfigured ? "configured" : "not configured"}.` : mode === "demo" ? "Offline demo uses a fixed three-step template and deterministic test agents. No LLM request is made." : "Real LLM planning and indexed ANS records. Execution requires fresh records and explicitly tested adapters; unrelated agents are not substituted."}</p>
          </section>
        ...
        {tab === "builder" && proposal && <>
          ...
          <div className="notice">{proposal.mode === "pilot" ? "Pilot catalog · Real LLM plan · Local test agents · Not registered with ANS" : proposal.mode === "demo" ? "Offline demo · Fixed template · Fixture agents · Not registered with ANS" : "Indexed ANS results · Real LLM plan · Identity has not been independently verified"}</div>
        ...
        {tab === "builder" && agent && <>
          ...
          <div className="notice">{agent.mode === "live" ? "Live workflow: agents receive the notes you submit. Identity remains unverified." : `${agent.mode === "pilot" ? "LLM-planned pilot" : "Offline demo"}: deterministic local agents analyze your supplied notes. No live research or verified identity is claimed.`}</div>
