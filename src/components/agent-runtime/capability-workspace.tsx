...
function Workspace({ local, userId }: { local: boolean; userId?: string }) {
  ...
  // An opened, saved agent is presented as its own app: its input, action and outputs come first.
  const agentMode=!!selected&&!proposal;
  ...
  function open(agent: Agent) {setSelected(agent);setProposal(undefined);setRun(undefined);setConfirmed(false);setAudio(undefined);setText('');setHistory([]);requestKey.current=undefined;window.history.replaceState(null,'',`/studio?agent=${encodeURIComponent(agent.revision?.rootId??agent.id)}`);window.scrollTo({top:0,behavior:'smooth'});}
  return <PageShell className="capability-workspace">
    ...
    {agentMode&&<p className="capability-back"><Button disabled={busy||running} onClick={()=>{setSelected(undefined);setRun(undefined);setHistory([]);window.history.replaceState(null,'','/studio');}}>← Create a new agent</Button></p>}
