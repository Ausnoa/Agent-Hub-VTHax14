...
export default function HostedDiscover({candidates,query,setQuery,busy,loading,error,searched,hasMore,search,add,publicAgents,publicAgentsError}:Props){
  ...
  // Client-side filter over the already-loaded saved templates â€” no request, so no submit button.
  ...
  function row(candidate:Candidate){return <article className="registry-item" key={candidate.agentId}>
    ...
    <div style={{minWidth:0,flex:1}}><h3>{candidate.name}</h3><p>{candidate.description||'No description provided by the agent owner.'}</p>
      <p className="mono" style={{marginTop:4}}>{candidate.skills.map(s=>s.name).join(' Â· ')}</p>
  ...
  return <PageShell className="screen-discovery hosted-discovery"><PageHeader eyebrow="AUTONOMOUS PIPELINE DISCOVERY" title="Discover your next capability." description="Explore the ANS registry, inspect agent capabilities, and find the right collaborators for your workflow." action={<Link className="btn btn-primary" href="/dashboard">Open dashboard <ArrowRight size={14}/></Link>}/>
    <div className="split-layout">
      <Card className="mesh-panel"><CardHead>Compose an agent</CardHead><p className="hint" style={{marginBottom:16}}>Have an outcome in mind? Describe it and Agent Glorria will suggest a workflow using your saved agents and this registry.</p>
        ...
        <p className="hint">Illustrative topology Â· search results appear in the registry panel.</p><Link className="btn btn-primary" href="/create" style={{marginTop:'auto'}}>Start composing <ArrowRight size={14}/></Link>
      </Card>
      <Card className="registry-panel"><CardHead badge={<Radar size={16}/>}>ANS capability resolution</CardHead>
        ...
        <section aria-label="Your saved agents"><h3 className="discover-section-title">Your saved agents</h3>
          ...
          {loading?<p role="status">Loading your agentsâ€¦</p>:
        ...
        <h3 className="discover-section-title">Search the live ANS registry</h3><p className="hint" style={{marginBottom:10}}>Queries the registry directly â€” no LLM key required. Up to 100 searches per UTC day.</p>
        <form className="search-bar" onSubmit={event=>{event.preventDefault();void search();}}><label className="sr-only" htmlFor="hosted-registry-query">Search ANS</label><input id="hosted-registry-query" value={query} maxLength={256} disabled={busy} onChange={e=>setQuery(e.target.value)} placeholder="Search for a capability"/><Button type="submit" variant="primary" disabled={busy}>{busy?'Searchingâ€¦':<><Search size={14}/> Search</>}</Button></form>
        <p role="status" className="hint">{busy?'Searching the ANS registryâ€¦':searched?`${external.length} registry results shown.`:''}</p>
    ...
    <Card style={{marginTop:20}}><CardHead>Discover public agents</CardHead>
      ...
      {!publicAgentsError&&publicAgents===undefined&&<p className="hint">Loadingâ€¦</p>}
