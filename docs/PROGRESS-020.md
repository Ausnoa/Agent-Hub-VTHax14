...
After user configured Vercel variables, found production still on the pre-hosted-builder commit while branch preview was successful but Vercel-login protected. All 52 tests passed. Merged hosted builder into main at d375ede and pushed; Vercel reports successful production deployment. Public /api/hosted/agents now returns 401 without a token (previously local-only 403). Browser on the canonical /agent-preview shows configured sign-in. Requested user sign-in on that origin to finish deployed save/test verification; localhost sessions are not transferred across origins. Public authenticated model execution remains unverified until that step.
