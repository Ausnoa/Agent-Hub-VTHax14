...
## Registry metadata survey — September 19, 2026

Surveyed 200 active A2A search records field by field, fetched four single-agent records, and crawled 5,800 records until the rate limit stopped the crawl.

- **Search records** (`GET /v1/ans/registered-agents`) include `agentId`, `ansName`, `agentHost`, `agentDisplayName`, `agentDescription`, `agentVersion`, `providerId` (occasionally absent), `lifecycle.status`, `expiresAt`, `indexedAt`, transparency-log `logId` and `leafIndex`, and `scores` (`trustScore`, `textScore`, `relevance`; results are sorted by `relevance`). Each endpoint has `protocol` (`A2A`, `HTTP-API`, `MCP`), `agentUrl`, usually `metaDataUrl` and `transports`, occasionally `documentationUrl`, and `functions` with `id`, `name`, and `tags`. ANS therefore already publishes declared skills; there is no function description field.
- **Single-agent records** (`GET /v1/ans/registered-agents/{agentId}`) add `trustScoreExplanation`, `trustVector` (`identity`, `integrity`), `missingness`, `coverage`, and per-signal evidence under `signals`: DNSSEC and TLSA status, DNS record consistency, expected versus observed certificate fingerprints, certificate type, domain validation, agent age, version stability, HTTPS records, and how ANS fetched the agent card.
- **Transparency log.** Each agent's DNS badge points to `https://transparency.ans.godaddy.com/v1/agents/{agentId}`, which returns a signed registration event with provisioned DNS records, certificate fingerprints, and a Merkle inclusion proof. This is the basis for identity checks the application could perform itself. For one agent the log reported `EXPIRED` while search still reported `ACTIVE`, and 6 of 200 search records were past `expiresAt` but `ACTIVE`, so `lifecycle.status` alone does not establish usability.
- **Limits.** `pageSize` accepts at most 100. `profile` accepts only `default`. There is no POST search. Responses carry `ratelimit-limit: 60`, `ratelimit-remaining`, and `ratelimit-reset` (seconds); the crawl received HTTP 429 after 58 back-to-back requests.
- **Data quality.** Of the first 5,800 records, 5,775 were `helpagent.club` templates across 15 distinct domains. Two early records broke the original normalizer: an A2A endpoint with no `transports`, and a relative `metaDataUrl`. Both are now tolerated, and invalid records are skipped instead of failing their page.
