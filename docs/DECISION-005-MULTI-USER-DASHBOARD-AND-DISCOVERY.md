...
- New hosted-only pages (`/dashboard`, `/saved`, `/profile/[username]`, `/profile/settings`) are self-contained components using the existing `useAccount()`/`hostedApi()` pattern (own sign-in prompt, own "hosted not configured" state), added to `WorkspaceAccess`'s always-passthrough list rather than given bespoke local/hosted branches.
